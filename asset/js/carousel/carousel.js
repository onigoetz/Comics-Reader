...
function Carousel(source)
{
	...
	function currentPaneOffset() {
        ...
        return offset = -((100/container_width)*current_offset);
	}
        ...
        switch(ev.type) {
			...
			case 'tap':
				...
				if($.contains(toolbar[0], ev.target)) {
					...
					$target = $(ev.target.nodeName == "I"? ev.target.parentNode : ev.target);
    ...
    this.init = function() {
		...
		hammertime = new Hammer(element[0], { drag_lock_to_axis: true })
			.on("dragleft dragright release swipeleft swiperight tap", handleTouch)
